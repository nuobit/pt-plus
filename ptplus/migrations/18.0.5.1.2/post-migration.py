...
def migrate(env, version):
    ...
    replace_tags(
        env,
        "all",
        select_tags(env, "TMP DMIS BASE"),
        env.ref("ptplus.tax_tag_is_dm_04_07"),
    )
    replace_tags(
        env,
        "all",
        select_tags(env, "TMP DMIS TAX"),
        env.ref("ptplus.tax_tag_is_dm_04_08"),
    )

    # Delete the temporary tags
    env["account.account.tag"].search(
        [
            ("name", "in", ["TMP DMIS TAX", "TMP DMIS BASE"]),
        ]
    ).unlink()
