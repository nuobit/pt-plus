...
def migrate(cr, version):
    ...
    if new_eacs:
        ...
        for new_record in new_eacs:
            ...
            for old_record in old_eacs:
                ...
                # id -> position [0]
                if new_record[5] == old_record[5] and len(new_record[4]) == len(
                    old_record[4]
                ):
                    cr.execute(
                        """
                        UPDATE l10n_pt_company_eac
                        SET name = %s, code = %s, type = %s, parent_id = %s
                        WHERE id = %s;
                        """,
                        (
                            new_record[5],
                ...
                if new_record[4] == old_record[4]:
                    cr.execute(
                        ...
                        WHERE id = %s;
                        """,
                        (new_record[4], new_record[6], new_record[1], old_record[0]),
                    )
                    to_delete = True
                    continue

            # To prevent duplicates, delete new ones that were found in old the list
            if to_delete:
                try:
                    cr.execute(
                        """
                    ...
                    _logger.warning(
                        f"Could not drop record: {new_record[5]}"
    ...
    try:
        ...
        _logger.warning(
            f"Could not drop 'code_tmp' column. "
            f"It may not exist or another error occurred: {e}"
        )
